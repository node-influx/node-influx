...
describe('query builder', () => {
    describe('measurement builder', () => {
        it('builds with only name', () => {
            helpers_1.expect(new index_1.Measurement()
                .name('my_"meas')
    ...
    describe('expression builder', () => {
        it('creates basic queries', () => {
            helpers_1.expect(new index_1.Expression()
                .tag('my_"tag')
                .equals
                .value('42')
        ...
        it('throws when using a flagged regex', () => {
            helpers_1.expect(() => new index_1.Expression().field('f').matches.value(/a/i))
                .to.throw(/doesn't support flags/);
        });
        it('throws when using un-stringifyable object', () => {
            helpers_1.expect(() => new index_1.Expression().field('f').equals.value(Object.create(null)))
        ...
        operationsTable.forEach(({ method, yields }) => {
            it(`yields ${yields} from .${method}`, () => {
                helpers_1.expect(new index_1.Expression().field('f')[method].value(true).toString())
