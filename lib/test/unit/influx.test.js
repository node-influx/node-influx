...
describe('influxdb', () => {
    describe('constructor', () => {
        it('uses default options', () => {
            helpers_1.expect((new src_1.InfluxDB()).options).to.deep.equal({
                username: 'root',
                password: 'root',
                database: null,
                pool: undefined,
        ...
        it('parses dsns', () => {
            helpers_1.expect((new src_1.InfluxDB('https://connor:password@192.168.0.1:1337/foo')).options).to.deep.equal({
                username: 'connor',
                password: 'password',
                database: 'foo',
                pool: undefined,
        ...
        it('parses single configs', () => {
            helpers_1.expect((new src_1.InfluxDB({ database: 'foo', host: '192.168.0.1' })).options).to.deep.equal({
                username: 'root',
                password: 'root',
                database: 'foo',
                pool: undefined,
        ...
        it('parses cluster configs', () => {
            helpers_1.expect((new src_1.InfluxDB({ database: 'foo', hosts: [{ host: '192.168.0.1' }] })).options).to.deep.equal({
                username: 'root',
                password: 'root',
                database: 'foo',
                schema: [],
        ...
        it('parses parses schema', () => {
            ...
            helpers_1.expect(client.schema.my_db.my_measurement).to.be.defined;
    ...
    describe('methods', () => {
        ...
        const expectQuery = (method, options, httpMethod = 'POST', yields = { results: [{}] }) => {
            ...
            expectations.push(() => {
                helpers_1.expect(pool[method]).to.have.been.calledWith({
                    method: httpMethod,
                    path: '/query',
                    query: Object.assign({
                        u: 'root',
        ...
        const expectWrite = (body, options) => {
            ...
            expectations.push(() => {
                helpers_1.expect(pool.discard).to.have.been.calledWith({
                    method: 'POST',
                    path: '/write',
                    body,
                    query: Object.assign({
        ...
        it('.getDatabaseNames()', () => {
            ...
            return influx.getDatabaseNames().then(names => {
                helpers_1.expect(names).to.deep.equal(['_internal', 'influx_test_gen']);
            });
        });
        it('.getMeasurements()', () => {
            setDefaultDB('mydb');
            ...
            return influx.getMeasurements().then(names => {
                helpers_1.expect(names).to.deep.equal(['series_0', 'series_1', 'series_2']);
            });
        });
        it('.getSeries() from all', () => {
            ...
            return influx.getSeries().then(names => {
                helpers_1.expect(names).to.deep.equal([
                    'series_0,my_tag=0',
                    'series_0,my_tag=1',
                    'series_0,my_tag=5',
                    'series_0,my_tag=6',
        ...
        it('.getUsers()', () => {
            ...
            return influx.getUsers().then(names => {
                helpers_1.expect(names.slice()).to.deep.equal([
        ...
        describe('.grantPrivilege()', () => {
            ...
            it('throws if DB unspecified', () => {
                helpers_1.expect(() => influx.grantPrivilege('con"nor', 'READ')).to.throw(/default database/);
            });
        ...
        describe('.revokePrivilege()', () => {
            ...
            it('throws if DB unspecified', () => {
                helpers_1.expect(() => influx.revokePrivilege('con"nor', 'READ')).to.throw(/default database/);
            });
            it('fills in default DB', () => {
                setDefaultDB('my_\\"_db');
                expectQuery('json', 'revoke READ from "con\\"nor" on "my_\\"_db"');
        ...
        describe('.createContinuousQuery()', () => {
            ...
            it('throws if DB unspecified', () => {
                helpers_1.expect(() => influx.createContinuousQuery('my_"q', 'foo')).to.throw(/default database/);
            });
            it('fills in default DB', () => {
                setDefaultDB('my_"_db');
                expectQuery('json', 'create continuous query "my_\\"q" on "my_\\"_db" begin foo end');
        ...
        describe('.dropContinuousQuery()', () => {
            ...
            it('throws if DB unspecified', () => {
                helpers_1.expect(() => influx.dropContinuousQuery('my_"q')).to.throw(/default database/);
            });
            it('fills in default DB', () => {
                setDefaultDB('my_"_db');
                expectQuery('json', 'drop continuous query "my_\\"q" on "my_\\"_db"');
        ...
        describe('.write methods', () => {
            ...
            it('runs raw queries', () => {
                ...
                return influx.queryRaw('select * from series_0').then(res => {
                    helpers_1.expect(res).to.deep.equal(helpers_1.dbFixture('selectFromOne'));
                });
            });
            it('parses query output', () => {
                ...
                return influx.query('select * from series_0').then(res => {
                    helpers_1.expect(res.slice()).to.deep.equal([
