...
export class InfluxDB {
  ...
  constructor(options?: any) {
    ...
    this._options.schema.forEach(schema => {
      ...
      if (!db) {
        throw new Error(
          `Schema ${schema.measurement} doesn't have a database specified,` + 'and no default database is provided!',
        );
      }
  ...
  public setPassword(username: string, password: string): Promise<void> {
    return this._pool
      .json(
        this._getQueryOpts(
          {
            q: `set password for ${grammar.escape.quoted(username)} = ` + grammar.escape.stringLit(password),
          },
          'POST',
        ),
      )
  ...
  public grantPrivilege(
    ...
    return this._pool
      .json(
        this._getQueryOpts(
          {
            q: `grant ${privilege} on ${grammar.escape.quoted(database)} ` + `to ${grammar.escape.quoted(username)}`,
          },
          'POST',
        ),
      )
   ...
   * influx.dropShard(3)
   */
  public dropShard(shardid: number): Promise<void> {
    return this._pool
      .json(
        this._getQueryOpts(
          {
            q: `drop shard ${shardid}`,
          },
          'POST',
        ),
      )
   ...
   * @return
   * @example
   * influx.showShards('database').then(shards => {
  ...
  public writePoints(points: IPoint[], options: IWriteOptions = {}): Promise<void> {
    const { database = this._defaultDB(), precision = 'n' as grammar.TimePrecision, retentionPolicy } = options;
    ...
    return this._pool.discard({
      ...
      method: 'POST',
      path: '/write',
      query: { db: database, p: this._options.password, precision, rp: retentionPolicy, u: this._options.username },
    });
  }
   ...
   * ])
   */
  public writeMeasurement(measurement: string, points: IPoint[], options: IWriteOptions = {}): Promise<void> {
   ...
   * })
   */
  public query<T>(query: string | string[], options: IQueryOptions = {}): Promise<IResults<T> | IResults<T>[]> {
  ...
  private _defaultDB(): string {
    if (!this._options.database) {
      throw new Error(
        'Attempted to run an influx query without a default' + ' database specified or an explicit database provided.',
      );
    }
