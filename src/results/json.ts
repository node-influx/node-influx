...
function assertContainsNoError(res: IResponse) {
...
/**
 * The JSONParser parses JSON-formatted response from Influx. Generally
 * we prefer csv since it's easier to parse and more stream-friendly; this
 * is only used on older Influx versions.
 * @private
 */
export class JSONParser implements IParser {
  public assertNoErrors(res: IncomingMessage): Promise<void> {
    return this.marshal(res).then(assertContainsNoError);
  }

  public parse<T>(res: IncomingMessage, precision?: TimePrecision): Promise<IResults<T>[] | IResults<T>> {
    return this.marshal(res).then(body => {
      assertContainsNoError(body);

      if (body.results.length === 1) { // normalize case 3
        return parseInner(body.results[0].series, precision);
      } else {
        return body.results.map(result => parseInner(result.series, precision));
      }
    });
  }
  ...
  private marshal(res: IncomingMessage): Promise<IResponse> {
