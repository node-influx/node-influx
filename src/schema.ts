...
export class Schema {
  ...
  public coerceFields(fields: FieldMap): [string, string][] {
    ...
    if (consumed !== keys.length) {
      ...
      throw new Error(
        'Extraneous fields detected for writing InfluxDB point in ' + `${this._ref()}: \`${extraneous.join('`, `')}\`.`,
      );
    }
  ...
  public checkTags(tags: { [tag: string]: string }): string[] {
    ...
    if (extraneous.length > 0) {
      throw new Error(
        'Extraneous tags detected for writing InfluxDB point in ' + `${this._ref()}: \`${extraneous.join('`, `')}\`.`,
      );
    }
