...
const queries = [
  ...
  update("create user \"john\" with password 'password' with all privileges", {
    db,
  }),
  update("create user \"steve\" with password 'password'", { db }),
  fixture("showUsers", "show users", { db }),

  write(fs.readFileSync(path.join(__dirname, "fixtures_test_data.txt")), {
    db,
  }),
...
function write(body, params) {
  return () =>
    fetch(`${influxHost}/write?${querystring.stringify(params)}`, {
      method: "POST",
      body,
    });
}
...
function query(query, params = {}) {
  params.q = query;
  return () =>
    fetch(`${influxHost}/query?${querystring.stringify(params)}`, {
      method: "GET",
    }).then((res) => res.json());
}
