...
function write(body, params) {
  return () => fetch(`${influxHost}/write?${querystring.stringify(params)}`, { method: 'POST', body });
}
...
function query(query, params = {}) {
  params.q = query;
  return () => fetch(`${influxHost}/query?${querystring.stringify(params)}`, { method: 'GET' }).then(res => res.json());
}
