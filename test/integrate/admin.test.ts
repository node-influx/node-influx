  ...
  describe('retention policies', () => {
    const expectPolicy = (policy: any): Promise<void> => {
      return db
        ...
        .then(rps => expect(rps.find(p => p.name === policy.name)).to.deep.equal(policy))
        .then(() => undefined);
    };
          ...
          expect(queries.slice()).to.deep.equal([
            {
              name: '7d_perf',
              query:
                'CREATE CONTINUOUS QUERY "7d_perf" ON ' +
  ...
  describe('shards', () => {
    ...
    it('drops shard', () => {
      return db
        ...
        .then(res => db.dropShard(res[0].id))
        .then(() => db.showShards('influx_test_db'))
        .then(result => expect(result[0]).to.not.contain(`id=${shard1}`));
    });
  });
});
