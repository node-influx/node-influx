...
describe('InfluxDB', function () {
  ...
  describe('create client', function () {
    it('should create an instance without error', function () {
      client = new Influx({host: info.server.host, port: info.server.port, username: info.server.username, password: info.server.password, database: info.db.name, retentionPolicy: info.db.retentionPolicy})
      dbClient = new Influx({host: info.server.host, port: info.server.port, username: info.server.username, password: info.server.password, database: info.db.name})
      failClient = new Influx({host: info.server.host, port: 6543, username: info.server.username, password: info.server.password, database: info.db.name})
      failoverClient = new Influx({hosts: [
          {host: '192.168.1.1'},
          {host: '192.168.1.2'},
          {host: '192.168.1.3'},
          {host: '192.168.1.4'},
    ...
    describe('configured using URLs', function () {
      it('should parse it when passed as `options`', function () {
        var urlClient = new Influx(
          'http://admin:fancierpassword@influx.foobar.com:1337/mydatabase'
        )

        assert.equal(urlClient._options.host, 'influx.foobar.com')
        assert.equal(urlClient._options.port, 1337)
        assert.equal(urlClient._options.username, 'admin')
        assert.equal(urlClient._options.password, 'fancierpassword')
        assert.equal(urlClient._options.database, 'mydatabase')
      })
      ...
      it('should parse them when passed in `hosts`', function () {
        ...
        assert.equal(hostsParsed[0].url, 'http://127.0.0.1:1337')
        assert.equal(hostsParsed[1].url, 'https://127.0.0.2:1338')
      })
    })
  })
  ...
  describe('#url', function () {
    it('should build a properly formatted url', function () {
      var url = client._url('query', { db: info.db.name, rp: info.db.retentionPolicy, precision: info.server.timePrecision })
