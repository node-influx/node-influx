...
describe('influxdb', () => {
  describe('constructor', () => {
    ...
    it('parses dsns', () => {
      expect((new InfluxDB('https://connor:password@192.168.0.1:1337/foo') as any)._options).to.deep.equal({
        username: 'connor',
        password: 'password',
        database: 'foo',
        pool: undefined,
  ...
  describe('methods', () => {
    ...
    it('.getUsers()', () => {
      ...
      return influx.getUsers().then(names => {
        expect(names.slice()).to.deep.equal([{ user: 'john', admin: true }, { user: 'steve', admin: false }]);
      });
    });
