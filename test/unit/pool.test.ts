...
describe('pool', () => {
  ...
  describe('request generators', () => {
    it('makes a text request', () => {
      return pool.text({ method: 'GET', path: '/pool/json' }).then(data => expect(data).to.equal('{"ok":true}'));
    });
    ...
    it('parses JSON responses', () => {
      return pool.json({ method: 'GET', path: '/pool/json' }).then(data => expect(data).to.deep.equal({ ok: true }));
    });
