...
describe('results', () => {
  ...
  it('parses a simple table of results', () => {
          ...
          series: [
            {
              name: 'test_series',
              columns: ['time', 'mean'],
              values: [['2016-09-25T16:12:51.787Z', 42], ['2016-09-25T16:34:31.999Z', 44]],
            },
          ],
        },
      ],
    ...
    expect(r.groups()).to.deep.equal([{ name: 'test_series', tags: {}, rows: r.slice() }]);
          ...
          series: [
            {
              name: 'test_series',
              columns: ['time', 'mean'],
              values: [['2015-08-18T00:00:00Z', 42], ['2015-08-18T00:06:00Z', 44]],
            },
          ],
        },
      ],
            ...
            series: [
              {
                name: 'test_series',
                columns: ['time', 'mean'],
                values: [[1474819971787, 42], [1474821271999, 44]],
              },
            ],
          },
        ],
            ...
            series: [
              {
                name: 'test_series',
                columns: ['time', 'mean'],
                values: [[1474819971787000, 42], [1474821271999000, 44]],
              },
            ],
          },
        ],
    ...
    expect(r.groups()).to.deep.equal([
      {
        name: 'test_series',
        tags: { tag: 'a' },
        rows: [{ tag: 'a', mean: 1 }, { tag: 'a', mean: 2 }],
      },
      {
        name: 'test_series',
        tags: { tag: 'b' },
        rows: [{ tag: 'b', mean: 3 }, { tag: 'b', mean: 4 }],
      },
    ]);

    expect(r.group({ tag: 'a' })).to.deep.equal([{ tag: 'a', mean: 1 }, { tag: 'a', mean: 2 }]);

    expect(r.group({ tag: 'b' })).to.deep.equal([{ tag: 'b', mean: 3 }, { tag: 'b', mean: 4 }]);
